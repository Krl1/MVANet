...
def main(item):
    ...
    pretrained_dict = torch.load(
        os.path.join(ckpt_path, item + ".pth"), map_location="cuda"
    )
    ...
    with torch.no_grad():
        for name, root in to_test.items():
            ...
            for idx, img_name in enumerate(img_list):
                ...
                img_var = Variable(
                    img_transform(img_resize).unsqueeze(0), volatile=True
                ).cuda()

                prediction = infer(net, img_var, w_, h_)

                foreground = img.convert("RGBA")
                foreground.putalpha(prediction)

                if args["save_results"]:
                    check_mkdir(os.path.join(output_path, item, name))
                    foreground.save(
                        os.path.join(output_path, item, name, img_name[0] + ".png")
                    )
